...
interface Term {
  type: 'phrase' | 'proximity' | 'word' | 'wildcard';
  value: string;
...
const COMMON_OCR_SWAPS: OCRSwap[] = [
...
interface TermRowProps {
  term: Term;
  index: number;
  onUpdate: (index: number, newTerm: Term) => void;
...
function Panel({ title, children, defaultOpen = false, icon }: PanelProps) {
...
export default function FultonSearchBuilder() {
  ...
  const updateTerm = (index: number, newTerm: Term) => {
    const updated = [...terms];
  ...
  return (
    <>
      ...
      <div style={{
        maxWidth: "760px", margin: "0 auto", padding: "120px 24px 60px",
      }}>
        {/* Header */}
